...
export default function ModalRefertilizacion({ onClose, onSuccess }: ModalRefertilizacionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fertilizantesValidos.length === 0) {
      alert('Debe agregar al menos un fertilizante con fuente y dosis')
      return
    }
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Error al registrar refertilización')
    } finally {
      setLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Refertilización</h2>
        </div>
        <button
          onClick={onClose}
          type="button"
