...
export default function ModalIngresoInsumos({ onClose, onSuccess, insumoPreseleccionadoId  }: ModalIngresoInsumosProps) {
  ...
  const [insumosSeleccionados, setInsumosSeleccionados] = useState<InsumoSeleccionado[]>([
    { id: crypto.randomUUID(), insumoId: '', nombre: '', unidad: '', cantidad: '' }
  ])
...
useEffect(() => {
  ...
  const handleInsumoChange = (id: string, insumoId: string) => {
    ...
    setInsumosSeleccionados(prev =>
      prev.map(item =>
        item.id === id
          ? { ...item, insumoId, nombre: insumo?.nombre || '', unidad: insumo?.unidad || '' }
          : item
      )
    )
  }
          ...
          {loadingInsumos ? (
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-2 border-2 border-dashed border-blue-300 rounded-lg text-blue-600 hover:bg-blue-50 transition-colors flex items-center justify-center gap-2 text-sm font-medium"
              >
                <span>+</span>
              ...
              {!mostrarCrearInsumo ? (
                ...
                <div className="mt-3 p-4 bg-white rounded-lg border border-blue-200 space-y-3">
