  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const insumo of insumosValidos) {
      ...
      if (cantidad > insumo.stockDisponible) {
        toast.error(`No hay suficiente stock de ${insumo.nombre}. Disponible: ${insumo.stockDisponible} ${insumo.unidad}`)
        return
      }
    }
    ...
    try {
      ...
      for (const insumo of insumosValidos) {
        const response = await fetch('/api/insumos/movimientos', {
          ...
          body: JSON.stringify({
            ...
            loteId: loteId || null,
            notas: descripcionBase,  // Usamos la descripción enriquecida con módulo si aplica
          }),
        })
      ...
      console.error('Error:', error)
      toast.error('Error al registrar el uso de insumos')
    } finally {
      setLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="p-4 sm:p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Uso de Insumos</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
