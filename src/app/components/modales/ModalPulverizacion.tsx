// Modal de Pulverización - Aplicación de productos fitosanitarios a cultivos
'use client'
...
export default function ModalPulverizacion({ onClose, onSuccess }: ModalPulverizacionProps) {
  ...
  // Botón para pulverizar todas las hectáreas
  const handlePulverizarTodo = () => {
  ...
  const agregarProducto = () => {
    setProductos([...productos, { producto: '', dosis: '', unidad: 'L/ha' }])
  }
  ...
  // Actualizar producto
  const actualizarProducto = (index: number, campo: keyof Producto, valor: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que al menos un producto tenga nombre y dosis
    const productosValidos = productos.filter(
      p => p.producto.trim() && p.dosis.trim()
    )
...
if (hectareas) {
  descripcionFinal += ` - ${hectareasNum} ha`
      ...
      let notasCompletas = ''
      if (productosValidos.length > 0) {
        notasCompletas = 'Productos aplicados:\n'
        productosValidos.forEach((p, i) => {
          notasCompletas += `${i + 1}. ${p.producto}: ${p.dosis} ${p.unidad}\n`
        })
      }
      if (notas.trim()) {
    ...
    potreros.map((lote) => {
      ...
      return (
        ...
        <div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-cyan-500 hover:text-cyan-600 transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex gap-3 mt-6">
        <button
          type="button"
          onClick={onClose}
          className="flex-1 px-4 py-3 border border-gray-300 rounded-lg text-cyan-600 hover:bg-gray-50 font-medium"
        >
          Cancelar
        </button>
        <button
          type="submit"
          disabled={loading}
          className="flex-1 px-4 py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
        >
