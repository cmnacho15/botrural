...
interface ModalDividirOvinosSexadoProps {
  isOpen: boolean;
  onClose: () => void;
  potreros: Potrero[];
  onSuccess: () => void;
...
export default function ModalDividirOvinosSexado({
  isOpen,
  onClose,
  potreros,
  onSuccess,
}: ModalDividirOvinosSexadoProps) {
  ...
  const handleConfirmar = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/recategorizacion/dividir-ovinos-sexado", {
        method: "POST",
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900">
            🐑 Dividir Corderos Mamones por sexo
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-gray-600">
                      Total: {potrero.cantidad} Corderos Mamones
                    </span>
                  </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <p className="text-sm text-gray-700">
              ℹ️ Los "Corderos Mamones" se eliminarán y crearán nuevos registros
              de Corderos DL y Corderas DL
            </p>
          </div>
        </div>
