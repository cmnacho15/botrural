...
export default function SignOutPage() {
  ...
  const cerrarSesion = async () => {
    ...
    // Esto es lo que realmente funciona en Next.js 14 + App Router + NextAuth v4
    await signOut({ redirect: false })
    
    // Forzamos redirección y limpieza total de sesión
    router.push('/login')
    router.refresh()
  }
      ...
      <div className="bg-white p-10 rounded-2xl shadow-2xl max-w-md w-full text-center">
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={cargando}
            className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-70 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl transition flex items-center justify-center gap-3"
          >
