...
// Componente de tarjeta de dato individual
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-4 border border-gray-100">
      <div className="flex items-start gap-4">
        {/* Icono */}
        <div className={`${colorClasses[dato.color] || "bg-gray-500"} w-12 h-12 rounded-full flex items-center justify-center text-2xl flex-shrink-0`}>
          {dato.icono}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start mb-1">
            ...
            <span className="text-xs text-gray-500 whitespace-nowrap ml-2">
              {formatFecha(dato.fecha)}
            </span>
          </div>
          
          <p className="text-gray-700 text-sm mb-2">{dato.descripcion}</p>
  ...
  if (datos.length === 0) {
    return (
      <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
        ...
        <p className="text-gray-500">Comienza agregando eventos, gastos o movimientos de insumos</p>
      </div>
    );
  }
